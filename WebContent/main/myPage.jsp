...
<%@page import="com.main.db.DBconn" %>
<%@page import="com.main.fileupload.fileuploadDTO" %>
...
%>
    ...
    <section id="content">
      <section id="content">
        <section id="userProfile">
          ...
          <div id="profileImage">
            <img src="/XXProject/upload/<%=mem_profilePic %>" alt="<%=name %>"/>
          </div>
        </section>
	...
	try {
		...
		if(conn != null){
			...
			String sql = "SELECT * FROM tb_fileupload WHERE fu_email=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, email);
			rs = pstmt.executeQuery();
			if(rs.next()){
				fu_idx = rs.getInt("fu_idx");
				fu_name = rs.getString("fu_name");
				fu_category = rs.getString("fu_category");
				fu_price = rs.getString("fu_price");
				fu_comment = rs.getString("fu_comment");
				fu_filename1 = rs.getString("fu_filename1");
				fu_filename2 = rs.getString("fu_filename2");
				fu_filename3 = rs.getString("fu_filename3");
				fu_filename4 = rs.getString("fu_filename4");
				fu_filename5 = rs.getString("fu_filename5");
				fu_filepath = rs.getString("fu_email");
				fu_email = rs.getString("fu_email");
			}
		}
	}catch(ClassNotFoundException e){
		e.printStackTrace();
	...
	System.out.println("fu_email : "  + fu_email);
	}
	%>
       <section class="cards-wrap" style="width:677px;">
    	...
    	<c:set var="fileList" value="${fboardDAO.selectListAll(keyWord)}"/>
	        ...
	        <article class="card-top ">
		            ...
		            <div class="card-desc">
		                <h2 class="card-title"> <p>${item.name} ${item.email}</p></h2>
